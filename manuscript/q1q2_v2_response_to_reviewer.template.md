...
{{claim:DESIGN.DEVELOPMENT_PATIENTS|integer}} unique labeled patients.
BraTS-Africa contributes
{{claim:DESIGN.EXTERNAL_CONFIRMATORY_PATIENTS|integer}} confirmatory glioma
patients and {{claim:DESIGN.EXTERNAL_SUPPORTIVE_PATIENTS|integer}} separately
...
{{claim:DESIGN.FOLDS|integer}} validation folds, and every main model uses the
same {{claim:DESIGN.SEEDS|integer}} training seeds.
<!-- END_ARTIFACT_BOUND_RESULTS -->
...
{{claim:DESIGN.DEVELOPMENT_RUNS|integer}} development runs.
<!-- END_ARTIFACT_BOUND_RESULTS -->
...
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.FIRST_MEAN|3f}} and
...
**{{claim:METHOD.SELECTED_LOSS|raw}}**; external labels and the legacy internal
...
**New evidence.**
<!-- BEGIN_ARTIFACT_BOUND_RESULTS -->
U-Net+RES contains {{claim:RESOURCE.UNET_RES.PARAMETER_COUNT|integer}}
parameters, requires
...
{{claim:DESIGN.EXTERNAL_MAIN_CHECKPOINTS|integer}} frozen checkpoints. The
confirmatory cohort contains
{{claim:DESIGN.EXTERNAL_CONFIRMATORY_PATIENTS|integer}} glioma patients; other
