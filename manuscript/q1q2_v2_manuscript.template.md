...
**Short title:** Capacity-controlled evaluation of BU-Net components

**Authors:** [AUTHOR NAMES AND ORDER — CONFIRMATION REQUIRED]

**Affiliations:** [AFFILIATIONS — CONFIRMATION REQUIRED]

**Corresponding author:** [NAME, POSTAL ADDRESS, AND EMAIL — CONFIRMATION REQUIRED]

## Abstract

### Background

Residual Extended Skip (RES) and Wide Context (WC) were introduced as
components of BU-Net, but an apparent improvement over a smaller U-Net does
not by itself identify a component effect. Differences in parameter count,
compute, convergence, and random initialization can produce the same
observation.

### Purpose

To test whether the published RES component retains a measurable benefit
after parameter and compute matching, and to characterize uncertainty,
lesion-level failure, external domain shift, and measured resource cost.

### Materials and Methods

BraTS 2020 supplied
{{claim:DESIGN.DEVELOPMENT_PATIENTS|integer}} unique labeled development
patients. Models were trained with patient-level
{{claim:DESIGN.FOLDS|integer}}-fold cross-validation and the same
{{claim:DESIGN.SEEDS|integer}} seeds. The frozen matrix contained
{{claim:DESIGN.MODELS|integer}} models and
{{claim:DESIGN.DEVELOPMENT_RUNS|integer}} development runs. After model,
checkpoint, endpoint, and analysis freeze, one external session evaluated
BraTS-Africa: {{claim:DESIGN.EXTERNAL_CONFIRMATORY_PATIENTS|integer}} glioma
patients formed the confirmatory cohort and
{{claim:DESIGN.EXTERNAL_SUPPORTIVE_PATIENTS|integer}} patients with other
neoplasms formed a separately labeled supportive cohort. The primary endpoint
was patient-level mean WT, TC, and ET Dice. The primary contrast compared
U-Net+RES with a plain parameter-matched U-Net using paired patient bootstrap
intervals, paired sign-flip tests, Holm correction, and hierarchical
...
The study provides capacity-controlled, equal-seed, externally evaluated
evidence about published BU-Net components rather than proposing a new RES or
WC module. Interpretation is bounded to the frozen cohorts, models, endpoints,
and practical threshold.

**Keywords:** brain tumor segmentation; magnetic resonance imaging; U-Net;
BU-Net; external testing; reproducibility; capacity matching
...
{{claim:DESIGN.DEVELOPMENT_PATIENTS|integer}} development patients were
assigned deterministically to {{claim:DESIGN.FOLDS|integer}} folds stratified
...
**{{claim:METHOD.SELECTED_LOSS|raw}}**. Its executable alpha, beta, gamma,
...
{{claim:METHOD.EFFECTIVE_BATCH_SIZE_NATIVE_2D|integer}}. One warmup-cosine
schedule was used; no second scheduler modified the same learning rate.
Validation occurred every
{{claim:METHOD.VALIDATION_FREQUENCY_OPTIMIZER_STEPS|integer}} optimizer steps.

Convergence runs allowed at most
{{claim:METHOD.MAXIMUM_OPTIMIZER_STEPS|integer}} optimizer steps. Early
...
{{claim:METHOD.EARLY_STOPPING_MINIMUM_DELTA|raw}} within
...
{{claim:METHOD.COMPUTE_MATCHED_HOURS_PER_RUN|raw}} hours per run rather than at
an equal epoch count.

Every main model used the same {{claim:DESIGN.SEEDS|integer}} seeds in every
...
Gate G required all {{claim:DESIGN.DEVELOPMENT_RUNS|integer}} development runs,
...
{{claim:DESIGN.EXTERNAL_MAIN_CHECKPOINTS|integer}} frozen main checkpoints on
...
{{claim:METHOD.PAIRED_BOOTSTRAP_RESAMPLES|integer}} resamples and confidence
level {{claim:METHOD.CONFIDENCE_LEVEL|percent1}}. A two-sided paired sign-flip
test used {{claim:METHOD.PAIRED_PERMUTATION_RESAMPLES|integer}} Monte Carlo
resamples. The primary and four prespecified secondary contrasts formed one
Holm family with alpha {{claim:METHOD.MULTIPLICITY_ALPHA|raw}}.

The practical interpretation threshold was
{{claim:METHOD.PRACTICAL_THRESHOLD|3f}} mean regional Dice. It was not called
...
All {{claim:DESIGN.DEVELOPMENT_PATIENTS|integer}} development patients were
assigned to exactly one validation fold. The external confirmatory analysis
contained {{claim:DESIGN.EXTERNAL_CONFIRMATORY_PATIENTS|integer}} glioma
patients, and the supportive analysis contained
{{claim:DESIGN.EXTERNAL_SUPPORTIVE_PATIENTS|integer}} other-neoplasm patients.
The complete main external matrix comprised
{{claim:DESIGN.EXTERNAL_MAIN_CHECKPOINTS|integer}} checkpoint evaluations.
...
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.MEDIAN_DIFFERENCE|3f}}.
The paired percentile interval extended from
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.PAIRED_BOOTSTRAP_LOWER_95|3f}}
to
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.PAIRED_BOOTSTRAP_UPPER_95|3f}}.
The standardized paired effect was
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.STANDARDIZED_PAIRED_EFFECT_DZ|3f}},
the Holm-adjusted p value was
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.HOLM_ADJUSTED_P|pvalue}},
and the probability of superiority was
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.PROBABILITY_OF_SUPERIORITY|percent1}}.
{{claim:CONTRAST.UNET_RES_VS_UNET_PARAMETER_MATCHED_RES.INTERPRETATION_TEXT|raw}}

### Resource realization

U-Net+RES contained
{{claim:RESOURCE.UNET_RES.PARAMETER_COUNT|integer}} parameters and required
...
{{claim:RESOURCE.UNET_RES.TRAINING_PEAK_FRAMEWORK_ALLOCATED_UNIFIED_MEMORY_BYTES_MAX|integer}}
bytes.
...
{{claim:RESOURCE.UNET_PARAMETER_MATCHED_RES.INFERENCE_END_TO_END_P50_SECONDS|3f}}
...
[AUTHOR CONFIRMATION REQUIRED: institutional determination regarding use of
public deidentified data, exemption/waiver language, and whether additional
...
[AUTHOR CONFIRMATION REQUIRED: CRediT roles]

## Use of generative AI and assisted technologies

[AUTHOR CONFIRMATION REQUIRED: disclose tools and scope according to the
selected journal’s current policy. Do not describe provenance audits as AI
detectors.]
